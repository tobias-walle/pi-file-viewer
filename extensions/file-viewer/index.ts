...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("review-file", {
    description:
      "Review a recent write/edit tool call in a reusable file viewer",
  ...
  pi.registerShortcut("alt+w", {
    description: "Select write/edit tool call to review",
    handler: async (ctx) => {
      await reviewFile(ctx);
    },
  });
...
class ReviewFileSelectComponent {
  ...
  private startSpinner(): void {
    ...
    this.spinnerTimer = setInterval(() => {
      ...
      this.spinnerIndex =
        (this.spinnerIndex + 1) %
        ReviewFileSelectComponent.spinnerFrames.length;
      this.options.onRequestRender();
    }, 90);
  }
